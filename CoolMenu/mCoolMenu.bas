...
''  Herman Eldering(aka BodyCheck):
''       -Solved disabled topmenu problem
...
'  rtItem.Left = rtItem.Left + m_SideBitmapWidth
  If pmd.fType And MFT_SEPARATOR Then
...
'    TextOffset = IIf(WndObj.RightToLeft, -1, 1)
    ...
    If bDisabled Then _
      Call DrawMenuText(hWnd&, hdc&, OffsetRect(rtText, TextOffset, TextOffset), pmds(CStr(dwItemData)).sMenuText, GetSysColor(COLOR_BTNHIGHLIGHT), Not pmd.bMainMenu, WndObj.RightToLeft)
...
' If click = the end of a menu loop: convert back from OWNERDRAWNED
  ...
  If Not (WndObj.HelpObj Is Nothing) Then
    ...
    'The POPUP help bug was found by Kayl.  Thank you Kayl
